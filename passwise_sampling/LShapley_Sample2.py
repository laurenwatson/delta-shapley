...
class LayeredShapley_Sample2():
    ...
    def __init__(self, trainset, testset, num_classes, params):
        """
        Args:
    ...
    def calculate_random_loss(self, num_classes):
        ...
        random_preds=np.ones(num_classes)*(1/num_classes)
        return loss(torch.tensor(np.array([random_preds])), torch.tensor([0]))
    ...
    def run(self, eval_datapoint_ids, params):
        ...
        """
        self.SVdf = pd.DataFrame(columns = sum([[str(i) + "_SV",str(i) + "_time",str(i) + "_layer"] for i in eval_datapoint_ids],[]))
        p = self.calculate_layer_probabilities(self.n)
        shapley_values = np.zeros(self.n)
        iter = 1
        while (not self.check_convergence_rolling(iter, eval_datapoint_ids)) and iter < 2 * 10e5:
            row_iteration = dict()
            if iter % 10 == 0:
                print("LS2 running in iteration {}".format(iter))
            for i in range(len(eval_datapoint_ids)):
                ...
                while (sum(np.isin(sample, datapoint)) > 0):
                    # sample again
                    sample = np.random.choice(self.n, size=layer, replace=False)
                ...
                row_iteration[str(datapoint) + "_layer"] = layer
        ...
        #SV_df = pd.DataFrame(data = self.SVs, columns = [str(i) for i in range(len(datapoints))])
            if os.path.exists(params.save_dir + "/Shapley_Values_Sampling2_FM_CNN_n100_2.csv"):
                os.remove(params.save_dir + "/Shapley_Values_Sampling2_FM_CNN_n100_2.csv")
            self.SVdf.to_csv(params.save_dir + "/Shapley_Values_Sampling2_FM_CNN_n100_2.csv")
        self.f.close()
